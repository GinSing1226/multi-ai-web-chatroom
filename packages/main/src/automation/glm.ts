/**
 * GLM (智谱清言) 平台自动化实现
 * GLM (ChatGLM/Zhipu) Platform Automation Implementation
 * 实现 GLM 对话的自动化操作 / Implements automated GLM conversation operations
 */

import { BasePlatformAutomation } from './base';
import { GLM_SELECTORS } from '@shared/constants/platform-selectors';
import { logService } from '../services/log.service';
import TurndownService from 'turndown';

/**
 * GLM 自动化类
 * GLM Automation Class
 */
export class GlMAutomation extends BasePlatformAutomation {
  protected readonly platformId = 'glm';
  protected readonly baseUrl = 'https://chat.z.ai';
  protected readonly selectors = GLM_SELECTORS;

  /**
   * 导航到新会话 / Navigate to new session
   * 点击"新聊天"按钮 / Click "New Chat" button
   ...
   * 从 URL 中提取 GLM 会话 ID / Extract GLM conversation ID from URL
   */
  async getCurrentConversationId(): Promise<string | null> {
    ...
    try {
      ...
      // GLM URL 格式: https://chat.z.ai/c/{conversationId}
      const urlMatch = url.match(/\/c\/([a-zA-Z0-9_-]+)/);
  ...
  async isGenerating(): Promise<boolean> {
    ...
    try {
      const result = await this.page.evaluate(() => {
        // 1. 检查停止按钮 - GLM 的停止按钮包含方形块 rect
        const stopButtons = document.querySelectorAll('button[aria-label="停止"]');
        const stopButtonsByRect = document.querySelectorAll('button:has(svg rect)');
        const stopButtonsBySquare = document.querySelectorAll('button:has(span.block)');
        const hasStopButton = stopButtons.length > 0 ||
                               stopButtonsByRect.length > 0 ||
                               stopButtonsBySquare.length > 0;

        // 2. 检查发送按钮 - 如果有发送按钮说明生成完成
        const sendButtons = document.querySelectorAll('button#send-message-button');
        ...
        const inputBox = document.querySelector('textarea#chat-input') as HTMLTextAreaElement;
        const isInputDisabled = inputBox ? inputBox.disabled : false;

        console.log(`[GLM isGenerating] hasStopButton=${hasStopButton}, hasSendButton=${hasSendButton}, hasLoadingIndicator=${hasLoadingIndicator}, isInputDisabled=${isInputDisabled}`);

        // 判断逻辑: 如果有任何生成中的迹象，返回 true
        const isGenerating = hasStopButton || hasLoadingIndicator || isInputDisabled;
    ...
    try {
      ...
      for (let i = 0; i < maxStableChecks; i++) {
        ...
        const currentCount = await this.page.evaluate(() => {
          return document.querySelectorAll('div.markdown-prose').length;
        });

        console.log(`[GLM] 等待渲染... 容器数量: ${currentCount} (上次: ${lastCount})`);

        if (currentCount === lastCount) {
          stableCount++;
          if (stableCount >= 2) {
            // 容器数量连续2次没有变化，认为渲染完成
            console.log(`[GLM] 容器数量稳定，准备提取`);
            break;
          }
        } else {
          stableCount = 0;
      ...
      const result = await this.page.evaluate(() => {
        // 查找所有 div.markdown-prose 容器（GLM 的消息容器）
        const containers = document.querySelectorAll('div.markdown-prose');

        console.log(`[Playwright] ===== 提取开始 =====`);
        console.log(`[Playwright] 找到 ${containers.length} 个 div.markdown-prose 容器`);
