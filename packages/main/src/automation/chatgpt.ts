/**
 * ChatGPT 平台自动化实现
 * ChatGPT Platform Automation Implementation
 * 实现 ChatGPT 对话的自动化操作 / Implements automated ChatGPT conversation operations
 */

import { BasePlatformAutomation } from './base';
import { CHATGPT_SELECTORS } from '@shared/constants/platform-selectors';
 ...
 */
export class ChatGPTAutomation extends BasePlatformAutomation {
  protected readonly platformId = 'chatgpt';
  protected readonly baseUrl = 'https://chatgpt.com';
  protected readonly selectors = CHATGPT_SELECTORS;
  ...
  async navigateToNewSession(): Promise<string> {
    ...
    try {
      ...
      logService.info(this.platformId, `当前 URL: ${currentUrl} / Current URL`);

      // 如果已经在首页，不需要再点击"新对话"按钮
      // If already on homepage, no need to click "New Chat" button
      if (currentUrl === this.baseUrl || currentUrl === `${this.baseUrl}/`) {
        logService.info(this.platformId, '已在首页，跳过点击新对话按钮 / Already on homepage, skip clicking new chat button');
   ...
   * 从 URL 中提取 ChatGPT 会话 ID / Extract ChatGPT conversation ID from URL
   */
  async getCurrentConversationId(): Promise<string | null> {
    ...
    try {
      ...
      // ChatGPT URL 格式: https://chatgpt.com/c/{uuid}
      const urlMatch = url.match(/\/c\/([a-f0-9-]{36})/);
   ...
   *
   * ChatGPT 特征 / ChatGPT Characteristics:
   * - 生成中: 停止按钮可见 (data-testid="stop-button")
   * - 已完成: 发送按钮可见 (data-testid="send-button") 或 听写按钮出现
   */
  async isGenerating(): Promise<boolean> {
    ...
    try {
      const result = await this.page.evaluate(() => {
        ...
        // 2. 检查听写按钮 - 完成后出现
        // Check dictation button - appears after completion
        const dictationButton = document.querySelector('button[aria-label="听写按钮"]');
        const hasDictationButton = dictationButton !== null;

        // 3. 检查发送按钮 - 生成完成时可见
        // Check send button - visible when generation completed
        const sendButton = document.querySelector('button[data-testid="send-button"]');
        const hasSendButton = sendButton !== null;

        console.log(`[ChatGPT isGenerating] hasStopButton=${hasStopButton}, hasDictationButton=${hasDictationButton}, hasSendButton=${hasSendButton}`);

        // 判断逻辑: 如果有停止按钮，说明正在生成
        // 如果有听写按钮或发送按钮，说明已完成
        // Determine logic: If has stop button, generating
        // If has dictation button or send button, completed
        const isGenerating = hasStopButton && !hasDictationButton;
  ...
  async extractResponse(): Promise<string> {
    ...
    try {
      ...
      for (let i = 0; i < maxStableChecks; i++) {
        ...
        console.log(`[ChatGPT] 等待渲染... 容器数量: ${currentCount} (上次: ${lastCount})`);

        if (currentCount === lastCount) {
          stableCount++;
          if (stableCount >= 2) {
            console.log(`[ChatGPT] 容器数量稳定，准备提取`);
            break;
          }
        } else {
          stableCount = 0;
