/**
 * Kimi (月之暗面) 平台自动化实现
 * Kimi (Moonshot) Platform Automation Implementation
 * 实现 Kimi 对话的自动化操作 / Implements automated Kimi conversation operations
 */

import { BasePlatformAutomation } from './base';
import { KIMI_SELECTORS } from '@shared/constants/platform-selectors';
import { logService } from '../services/log.service';
import TurndownService from 'turndown';

/**
 * Kimi 自动化类
 * Kimi Automation Class
 */
export class KimiAutomation extends BasePlatformAutomation {
  protected readonly platformId = 'kimi';
  protected readonly baseUrl = 'https://kimi.moonshot.cn';
  protected readonly selectors = KIMI_SELECTORS;
  ...
  async navigateToNewSession(): Promise<string> {
    ...
    try {
      ...
      try {
        ...
        // Kimi 特殊处理：如果点击新建按钮失败，直接跳过
        // Kimi special handling: If clicking new chat button fails, skip it
        logService.warn(this.platformId, '点击新建按钮失败，直接跳过（Kimi 特殊处理）/ Clicking new chat button failed, skipping (Kimi special handling)', clickError);
      }
   ...
   * 从 URL 中提取 Kimi 会话 ID / Extract Kimi conversation ID from URL
   */
  async getCurrentConversationId(): Promise<string | null> {
    ...
    try {
      ...
      // Kimi URL 格式: https://kimi.moonshot.cn/chat/{conversationId}
      const urlMatch = url.match(/\/chat\/([a-zA-Z0-9_-]+)/);
  ...
  async sendMessage(content: string, useDirectFill = false): Promise<void> {
    ...
    try {
      ...
      // 2. 清空输入框并输入内容
      // Kimi 使用 contenteditable div，需要用 fill 而不是 type
   ...
   * 通过按钮状态和生成指示器判断 / Determine via button state and generating indicator
   *
   * Kimi 特征 / Kimi Characteristics:
   * - 生成中: 显示停止按钮（stop SVG icon）
   * - 已完成: 显示发送按钮（Send icon，无论是否置灰都是已完成状态）
   * - 已完成状态包括: 发送按钮可点击、发送按钮置灰（disabled但还是Send icon）
   */
  async isGenerating(): Promise<boolean> {
    ...
    try {
      const result = await this.page.evaluate(() => {
        // 1. 检查是否有停止按钮（stop icon）- 这是唯一的生成中标志
        const stopButtons = document.querySelectorAll('svg[name="stop"]');
        const hasStopButton = stopButtons.length > 0;

        // 2. 检查是否有发送按钮（Send icon）- 无论是否置灰，有Send icon就说明已完成
        const sendButtons = document.querySelectorAll('svg[name="Send"]');
        const hasSendButton = sendButtons.length > 0;

        // 3. 检查生成指示器（作为备选判断）
        const loadingIndicators = document.querySelectorAll('[class*="loading"], [class*="generating"]');
        const hasLoadingIndicator = loadingIndicators.length > 0;

        // 4. 检查输入框状态 - 生成中输入框可能被禁用
        const inputBox = document.querySelector('div.chat-input-editor[contenteditable="true"]') as HTMLElement;
        const isInputDisabled = inputBox ? inputBox.getAttribute('contenteditable') === 'false' : false;

        console.log(`[Kimi isGenerating] hasStopButton=${hasStopButton}, hasSendButton=${hasSendButton}, hasLoadingIndicator=${hasLoadingIndicator}, isInputDisabled=${isInputDisabled}`);

        // 🔥 修正后的判断逻辑：
        // - 如果有停止按钮（stop icon），说明正在生成
        // - 如果有发送按钮（Send icon），说明已完成（无论是否置灰）
        // - 只有当有停止按钮 且 没有发送按钮时，才认为正在生成
        const isGenerating = hasStopButton && !hasSendButton;

        // 作为额外的保险，如果检测到 loading indicator 或输入框禁用，也认为在生成
        const isGeneratingFallback = hasLoadingIndicator || isInputDisabled;

        return isGenerating || isGeneratingFallback;
      });
  ...
  async extractResponse(): Promise<string> {
    ...
    try {
      ...
      for (let i = 0; i < maxStableChecks; i++) {
        ...
        const currentCount = await this.page.evaluate(() => {
          ...
          for (const selector of selectors) {
            ...
            if (containers.length > 0) {
              console.log(`[Kimi] 使用选择器 ${selector} 找到 ${containers.length} 个容器`);
        ...
        console.log(`[Kimi] 等待渲染... 容器数量: ${currentCount} (上次: ${lastCount})`);

        if (currentCount === lastCount) {
          stableCount++;
          if (stableCount >= 2) {
            // 容器数量连续2次没有变化，认为渲染完成
            console.log(`[Kimi] 容器数量稳定，准备提取`);
            break;
          }
        } else {
          stableCount = 0;
      ...
      const result = await this.page.evaluate(() => {
        // 尝试多个选择器查找 Kimi 的消息容器
        // Try multiple selectors to find Kimi's message containers
        const selectors = [
          'div.markdown-container',
          'div[class*="markdown"]',
          'div[class*="message"]',
        ...
        for (const selector of selectors) {
          ...
          if (found.length > 0) {
            ...
            console.log(`[Playwright] ===== 使用选择器: ${selector} =====`);
            console.log(`[Playwright] 找到 ${found.length} 个容器`);
            break;
          }
        }

        if (!containers || containers.length === 0) {
          console.log(`[Playwright] ❌ 所有选择器都未找到容器`);
          // 调试：打印页面上所有可能的元素
          console.log(`[Playwright] 🔍 调试信息：`);
          console.log(`[Playwright]   页面标题: ${document.title}`);
          console.log(`[Playwright]   页面 URL: ${window.location.href}`);
          return { containers: 0, html: '', selector: '' };
        }
        ...
        console.log(`[Playwright]   使用的选择器: ${usedSelector}`);
        console.log(`[Playwright]   HTML 长度: ${html.length}`);
        console.log(`[Playwright]   文本长度: ${text.length}`);
        console.log(`[Playwright]   文本预览（前300字符）:\n${text.substring(0, 300)}`);

        return { containers: containers.length, html, selector: usedSelector };
      });

      logService.debug(this.platformId, `提取到 HTML，总容器数: ${result.containers}, HTML长度: ${result.html.length}, 使用选择器: ${result.selector} / Extracted HTML`);
      ...
      if (!result.html || result.html.trim().length === 0) {
        ...
        logService.error(this.platformId, `页面结构调试信息: ${JSON.stringify(pageStructure)} / Page structure debug info`);
        return '';
      }
