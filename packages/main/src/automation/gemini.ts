/**
 * Gemini 平台自动化实现
 * Gemini Platform Automation Implementation
 * 实现 Gemini 对话的自动化操作 / Implements automated Gemini conversation operations
 */

import { BasePlatformAutomation } from './base';
import { GEMINI_SELECTORS } from '@shared/constants/platform-selectors';
 ...
 */
export class GeminiAutomation extends BasePlatformAutomation {
  protected readonly platformId = 'gemini';
  protected readonly baseUrl = 'https://gemini.google.com';
  protected readonly selectors = GEMINI_SELECTORS;
  ...
  async navigateToNewSession(): Promise<string> {
    ...
    logService.info(this.platformId as any, '开始导航到新会话 / Navigating to new session');
    ...
    try {
      ...
      try {
        ...
        // Gemini 特殊处理：如果点击新建按钮失败，直接跳过
        // Gemini special handling: If clicking new chat button fails, skip it
        logService.warn(this.platformId as any, '点击新建按钮失败，直接跳过（Gemini 特殊处理）/ Clicking new chat button failed, skipping (Gemini special handling)');
      }
      ...
      if (conversationId) {
        logService.info(this.platformId as any, `成功导航到会话: ${conversationId} / Successfully navigated to session`);
      ...
      logService.info(this.platformId as any, '新会话已创建 (会话 ID 将在发送消息后由轮询任务提取) / New session created (conversation ID will be extracted by polling task after sending message)');
      return '';
    } catch (error) {
   ...
   * 从 URL 中提取 Gemini 会话 ID / Extract Gemini conversation ID from URL
   */
  async getCurrentConversationId(): Promise<string | null> {
    ...
    try {
      ...
      if (urlMatch && urlMatch[1]) {
        ...
        logService.info(this.platformId as any, `✅ 提取到会话 ID: ${conversationId} / Extracted conversation ID`);
        return conversationId;
      }

      logService.warn(this.platformId as any, '未能从 URL 提取会话 ID / Failed to extract conversation ID from URL');
      return null;
    } catch (error) {
  ...
  async navigateToSession(conversationId: string): Promise<void> {
    ...
    try {
      ...
      const url = `${this.baseUrl}/app/${conversationId}`;
      ...
      logService.info(this.platformId as any, `成功导航到会话: ${conversationId} / Successfully navigated to session: ${conversationId}`);
    } catch (error) {
  ...
  async sendMessage(content: string, useDirectFill = false): Promise<void> {
    ...
    logService.info(this.platformId as any, `准备发送消息: ${content.substring(0, 50)}... / Preparing to send message: ${content.substring(0, 50)}...`);
    ...
    try {
      ...
      // Gemini 使用 contenteditable div，需要用 fill 而不是 type
      ...
      try {
        ...
        logService.info(this.platformId as any, '通过按钮发送消息 / Message sent via button');
      } catch (buttonError) {
        // 如果按钮点击失败，尝试使用 Enter 键 / If button click fails, try Enter key
        logService.warn(this.platformId as any, '按钮点击失败，尝试使用 Enter 键 / Button click failed, trying Enter key');
        await this.page.keyboard.press('Enter');
        logService.info(this.platformId as any, '通过 Enter 键发送消息 / Message sent via Enter key');
      }

      // 5. 等待发送完成 / Wait for send to complete
      await this.page.waitForTimeout(1000);

      logService.info(this.platformId as any, '消息已成功发送 / Message sent successfully');
    } catch (error) {
  ...
  async waitForResponse(timeout = 60000): Promise<string> {
    ...
    logService.info(this.platformId as any, `开始等待响应 (超时: ${timeout}ms) / Starting to wait for response (timeout: ${timeout}ms)`);
    ...
    try {
      ...
      while (Date.now() - startTime < Math.min(timeout, 30000)) {
        ...
        if (!isGenerating) {
          ...
          if (responseContent && responseContent !== lastContent) {
            logService.info(this.platformId as any, `检测到生成完成，响应长度: ${responseContent.length} / Generation completed, response length: ${responseContent.length}`);
      ...
      logService.warn(this.platformId as any, '停止按钮检测超时，启用内容稳定性判断 / Stop button detection timeout, enabling content stability check');
      ...
      if (isStable) {
        ...
        logService.info(this.platformId as any, `内容稳定，最终响应长度: ${finalContent.length} / Content stable, final response length: ${finalContent.length}`);
        return finalContent;
      }
   ...
   * 通过停止按钮和麦克风按钮判断 / Determine via stop button and mic button
   *
   * Gemini 特征 / Gemini Characteristics:
   * - 生成中: 停止按钮可见 (button.send-button.stop)
   * - 已完成: 麦克风按钮出现 (button.speech_dictation_mic_button)
   */
  async isGenerating(): Promise<boolean> {
    ...
    try {
      const result = await this.page.evaluate(() => {
        ...
        // 2. 检查麦克风按钮 - 完成后出现
        // Check mic button - appears after completion
        const micButton = document.querySelector('button.speech_dictation_mic_button');
        const hasMicButton = micButton !== null;

        console.log(`[Gemini isGenerating] hasStopButton=${hasStopButton}, hasMicButton=${hasMicButton}`);
    ...
    try {
      ...
      for (let i = 0; i < maxStableChecks; i++) {
        ...
        const currentCount = await this.page.evaluate(() => {
          return document.querySelectorAll('message-content .markdown.markdown-main-panel').length;
        });

        console.log(`[Gemini] 等待渲染... 容器数量: ${currentCount} (上次: ${lastCount})`);

        if (currentCount === lastCount) {
          stableCount++;
          if (stableCount >= 2) {
            // 容器数量连续2次没有变化，认为渲染完成
            console.log(`[Gemini] 容器数量稳定，准备提取`);
            break;
          }
        } else {
          stableCount = 0;
      ...
      const result = await this.page.evaluate(() => {
        const containers = document.querySelectorAll('message-content .markdown.markdown-main-panel');

        console.log(`[Playwright] ===== 提取开始 =====`);
        console.log(`[Playwright] 找到 ${containers.length} 个 message-content 容器`);
      ...
      if (!result.html || result.html.trim().length === 0) {
        logService.warn(this.platformId as any, '未能提取到任何内容 / Failed to extract any content');
        return '';
      }
      ...
      logService.info(this.platformId as any, `成功提取内容，长度: ${trimmedContent.length} / Successfully extracted content, length: ${trimmedContent.length}`);
